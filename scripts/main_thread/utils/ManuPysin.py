...
class ManuPysin(FunctionNameDecorator):
    ...
    @name
    def pixel_matches_color(self, pixel, color, tolerance=0):
        match = False
        current_rgb = self.pixel(*pixel)
        ...
        self.mp.print(f'expected_color: {color}, result: {current_rgb}, with tolerance: {match}')
        return match

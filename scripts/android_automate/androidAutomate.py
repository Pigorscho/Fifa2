...
class Device:
	...
	def inputText(self, text):
		...
		# """
		# text = text.replace(" ", "%s")
		command = f"adb -s {self.deviceId} shell input text '{text}'"
		os.system(command)
	...
	def enter(self):
		...
		# Function that presses the volume down button on your device
		# """
		os.system(f"adb -s {self.deviceId} shell input keyevent KEYCODE_ENTER")
	...
	def recordEvent(self, event):
		...
		Returns:
			tuple: (x, y) coordinates of the tap.
		"""
		if not os.path.exists("../../events"):  # make the events directory if it does not exist
			os.makedirs("../../events")

		# Capture the output of the getevent command
		cmd = f"adb -s {self.deviceId} shell getevent -t /dev/input/event{self.eventId}"
		...
		with open(f"./events/{event}", "w") as f:
			f.write(output)
	...
	def parseScreenXML(self):
		...
		# os.system(f"adb -s {self.deviceId} pull $(adb -s {self.deviceId} shell uiautomator dump | findstr -oP '[^ ]+.xml') ./screendump.xml")
		os.system(f"adb -s {self.deviceId} pull $(adb -a shell uiautomator dump | findstr -o '/.*?xml')")
...
class Emulator:
	...
	# """
	def __init__(self, emulatorId=False, deviceId=None, verbose=True):
